...
from PyQt5.QtWidgets import (QMainWindow, QApplication, QPushButton, QDialog, QVBoxLayout, 
...
categoryList=["Cars and vehicles", "Beauty and style", "Science and technology", "Comedy", "Sports", "Education", "Entertainment", "Filming and animation", "Finance and business", "Gastronomy", "Games", "Pets and animals", "Music", "News and politics", "NGOs and activism", "People and blogs", "Travel and events","Not available"]
channelList=[]
...
def fillList():
    try:
        ...
        msgBox.setText("The program couldn't open the file\nExcept: "+str(OSError))
...
class Window(QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QtGui.QIcon('img/fortilogo.png')) #FortiLogo
    ...
    #Configurar la interfaz de usuario gráfica != initModel() --> Backend
    def initGUI(self):
        #La tabla
        self.tableWidget=QTableWidget(self.centralWidget)
        self.tableWidget.setColumnCount(3)
        self.tableWidget.setColumnWidth(0,200) #Ancho de la columna 1
        self.tableWidget.setColumnWidth(1,480) #Ancho de la columna 2
        self.tableWidget.setColumnWidth(2,150) #Ancho de la columna 3
        ...
        self.tableWidget.setWordWrap(False) #Ajustar texto para tener visibilidad completa de la URL dentro del ancho de la columna 
        self.tableWidget.horizontalHeader().setStretchLastSection(True) #Llenar espacio disponible con la última columna en la fila de los títulos
        self.tableWidget.verticalHeader().setHighlightSections(False) #Deshabilitar resaltado del texto del encabezado al seleccionar una fila
        self.tableWidget.horizontalHeader().setHighlightSections(False) #Deshabilitar resaltado del texto del encabezado al seleccionar una fila
        #self.tableWidget.verticalHeader().setVisible(False) #Ocultar encabezado vertical (índices de la tabla)
        self.tableWidget.setAlternatingRowColors(True) #Dibujar el fondo de las filas alternando entre blanco y gris
        
        #Menú contextual personalizado para QTableWidget (Menú al hacer clic derecho en un elemento). Se puede personalizar para adaptarse a las necesidades del usuario y para proporcionar acceso rápido a las opciones de la aplicación
        ...
        #El botón de mostrar
        menu=QMenu()
        for index, column in enumerate(['Channel', 'Link', 'Category'], start=0):
            action=QAction(column, menu)
            action.setCheckable(True)
            action.setChecked(True)
            action.setData(index)
            menu.addAction(action)
        showButton=QPushButton("Hide", self)
        showButton.setFixedWidth(60)
        showButton.setMenu(menu)
        showButton.move(50, 20)
        menu.triggered.connect(self.showHeaders)
        
        #El botón de añadir
        self.addRowButton = QPushButton('Add channel', self.centralWidget)
        self.addRowButton.setGeometry(QRect(120, 20, 130, 30))
        self.addRowButton.clicked.connect(self.showWindowAdd)

        #El botón de buscar
        self.searchButton = QPushButton('Search channel', self.centralWidget)
        self.searchButton.setGeometry(QRect(260, 20, 140, 30))
        ...
        #El botón de conectar con youtube
        self.fillButton = QPushButton("Get channels I'm subscribed to", self.centralWidget)
        self.fillButton.setGeometry(QRect(700, 20, 250, 30))
        self.fillButton.clicked.connect(self.youtubeConnection)
    ...
    #Acción del menú mostrar/ocultar
    def showHeaders(self, action):
        column = action.data()
        if action.isChecked():
            self.tableWidget.setColumnHidden(column, False)
        else:
            self.tableWidget.setColumnHidden(column, True)
    ...
    def youtubeConnection(self):
        messageBox = QMessageBox(self)
        messageBox.setIcon(QMessageBox.Question)
        messageBox.setText("Continue only if you have access to the API or your own client_secret.json in the folder, if you don't, contact the developer to request access")
        messageBox.setWindowTitle("Action confirmation")
        messageBox.setStandardButtons(QMessageBox.Yes | QMessageBox.No)
        messageBox.setDefaultButton(QMessageBox.No)
        messageBox.button(QMessageBox.Yes).setText('Continue')
        messageBox.button(QMessageBox.No).setText('Back')
        messageBox.buttonClicked.connect(self.open_in_browser)
        messageBox.exec_()
    ...
    def searchRow(self):
        ...
        for indexChannel in range(len(channelList)):
            ...
            if similarity >= 0.7: #Si está por encima del umbral (threshold)
                ...
                channelsFound.append(channelFound)  
        if(notFound):
            msgBox = QMessageBox()
            msgBox.setIcon(QMessageBox.Information)
            msgBox.setGeometry(350, 350, 0, 0)
            ...
            window.tableWidget.setRowCount(0) #Limpiar tabla
            window.fillTable(channelsFound) #Poner en la tabla el canal
            self.close()
...
class WindowEdit(QDialog):
    ...
    #Método para añadir funcionalidad al botón de editar
    def editRow(self):
        #Obtenemos los datos de los widgets
        channel=self.inputChannel.text()
        link=self.inputLink.text()
